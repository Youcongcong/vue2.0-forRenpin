<template>
  <div :class="['c-modal', class]" v-show="show" transition="fade">
    <c-mask v-if="backdrop" v-show="show" @touchend.prevent="show = false" transition="fade"></c-mask>
    <div class="c-modal-content" v-show="show" transition="slide-up">
      <div class="c-modal-body">
        <slot></slot>
      </div>
      <c-pane class="c-modal-footer" v-if="_actions">
        <c-cell class="row">
          <c-flex v-for="action in _actions">
            <c-button :class="action.class" :type="action.type" @click="_click($key)">{{action.label}}</c-button>
          </c-flex>
        </c-cell>
      </c-pane>
    </div>

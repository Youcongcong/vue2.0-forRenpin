<template>
  <div class="c-loadmore">
    <div class="c-loadmore-content" :class="{ 'is-dropped': topDropped || bottomDropped}" :style="{ 'transform': 'translate3d(0, ' + translate + 'px, 0)' }">
      <slot name="top">
        <div class="c-loadmore-top" v-if="topMethod">
          <c-loading v-if="topStatus === 'loading'" class="c-loadmore-loading" :size="0.5"></c-loading>
          <span class="c-loadmore-text">{{ topText }}</span>
        </div>
      </slot>
      <slot></slot>
      <slot name="bottom">
        <div class="c-loadmore-bottom" v-if="bottomMethod">
          <c-loading v-if="bottomStatus === 'loading'" class="c-loadmore-loading" :size="0.5"></c-loading>
          <span class="c-loadmore-text">{{ bottomText }}</span>
        </div>
      </slot>
    </div>
  </div>
        ...
        if (id === this.uuid) {
          this.$nextTick(() => {
            if (this.scrollEventTarget === window) {
              ...
              this.scrollEventTarget.scrollTop += 65;
            }
            this.translate = 0;
          });
        }

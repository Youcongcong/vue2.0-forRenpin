<template>
  <div :class="['c-form', class]">
    <form @submit.prevent="submit" autocomplete="off" novalidate>
      ...
      <c-title class="c-form-title" v-if="title">{{title}}</c-title>
      <div class="c-form-cells" v-if="items">
        <c-form-cell v-for="(key, cell) in cells" :field="key" :model="cell" :value="items[key]" @mutate="_mutate(key, $arguments)"></c-form-cell>
      </div>
